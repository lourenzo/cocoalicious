...
- (NSArray *) requestDatesFilteredByTag: (DCAPITag *) tag {
    ...
    NSData *responseData = [self sendRequestForURI: apiURL usingCachePolicy: NSURLRequestUseProtocolCachePolicy response: &response error: &error];
...
- (NSArray *) requestPostsFilteredByTag: (DCAPITag *) tag count: (NSNumber *) count {
	...
	if ([response statusCode] != 200) {
		[self displayHTTPError: response];
		return nil;
	}
...
- (NSDate *) requestLastUpdateTime: (NSError **) error {
    ...
    NSData *responseData = [self sendRequestForURI: apiURL usingCachePolicy: NSURLRequestReloadIgnoringCacheData response: &response error: error];
...
- (void) addPost: (DCAPIPost *) newPost {
	...
	BOOL private = [newPost isPrivate];
...
- (NSData *) sendRequestForURI: (NSURL *) apiURL usingCachePolicy: (NSURLRequestCachePolicy) cachePolicy response: (NSHTTPURLResponse **) response error: (NSError **) error {
	NSMutableString *credentialedString = [[[NSMutableString alloc] init] autorelease];
	
	[credentialedString appendFormat: @"%@://%@:%@@%@", [apiURL scheme], [[self username] stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding legalURLCharactersToBeEscaped: kLEGAL_CHARACTERS_TO_BE_ESCAPED], [[self password] stringByAddingPercentEscapesUsingEncoding: NSUTF8StringEncoding legalURLCharactersToBeEscaped: kLEGAL_CHARACTERS_TO_BE_ESCAPED], [apiURL host]];	

	if ([apiURL path]) {
		[credentialedString appendString: [apiURL path]];
	}

	if ([apiURL query] && ![[apiURL query] isEqualToString: @""]) {
		[credentialedString appendFormat: @"?%@", [apiURL query]];
	}

	NSURL *credentialedURL = [NSURL URLWithString: credentialedString];
	
	NSMutableURLRequest *req = [NSMutableURLRequest requestWithURL: credentialedURL cachePolicy: cachePolicy timeoutInterval: kREQUEST_TIMEOUT_INTERVAL];
...
- (void) displayError: (NSError *) error {
	...
	[alert setMessageText: [NSString stringWithFormat: NSLocalizedString(@"Error: %@", @""), [error localizedDescription]]];	
	
	NSMutableString *reasonString = [[[NSMutableString alloc] init] autorelease];
	NSString *failureReason = [error localizedFailureReason];
	
	if (failureReason) {
		[reasonString appendString: failureReason];
	}
	...
	if ([[error domain] isEqualToString: NSURLErrorDomain]) {
		 ...
		 [reasonString appendString: [[error userInfo] objectForKey:NSErrorFailingURLStringKey]];
	}
	
	[alert setInformativeText: reasonString];
...
- (void) displayHTTPError: (NSHTTPURLResponse *) response {
	NSAlert *alert = [[[NSAlert alloc] init] autorelease];
	[alert addButtonWithTitle: NSLocalizedString(@"OK", @"")];
	
	int statusCode = [response statusCode];
	
	[alert setMessageText: [NSString stringWithFormat: NSLocalizedString(@"HTTP Error: %@ (%d)", @""), [NSHTTPURLResponse localizedStringForStatusCode: statusCode], statusCode]];	
	
	[alert setInformativeText: NSLocalizedString(@"Cocoalicious encountered an error while trying to communicate with the server.  The problem is most likely on the server side.", @"")];
